...
namespace PhenomenologicalLayout
{
    ...
    public partial class Page1b : Page
    {
        ...
        {
            AnimateImages(-100, 200, 2, false, sender);
        }
        ...
        void AnimateImages(double xPos, double yPos, double scaleFactor, bool IsBack, object sender)
        {
            ...
            foreach (FrameworkElement fe in hostPanel.Children)
            {
                if (fe != selected_fe)
                {
                    ...
                    if (!IsBack)
                        //translate from parent for consistency
                        pt = hostGrid.TranslatePoint(new Point(0, 0), fe.Parent as UIElement);

                    tt.BeginAnimation(TranslateTransform.YProperty, new DoubleAnimation(pt.X, TimeSpan.FromMilliseconds(300)), HandoffBehavior.Compose);
                    tt.BeginAnimation(TranslateTransform.XProperty, new DoubleAnimation(pt.Y, TimeSpan.FromMilliseconds(300)), HandoffBehavior.Compose);
                }
                else
                {
                    TransformGroup tg = fe.RenderTransform as TransformGroup;
