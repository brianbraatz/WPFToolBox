...
namespace AnimationBehaviors
{
	...
	{
        private void AddRect(object sender, MouseButtonEventArgs e)
        {
            Rectangle rect = new Rectangle();
            rect.Width = rect.Height = 20;
            rect.Fill = new SolidColorBrush(Colors.Pink);
            rect.Margin = new Thickness(5);
            rect.MouseDown += AddRect;

            pan.Children.Insert(0, rect);

            AnimationBehaviorHost.SetLayoutBehavior(rect, LayoutBehavior.Springy);
        }

        private void NextPage(object sender, MouseButtonEventArgs e)
        {
            NavigationService.GetNavigationService(this).Navigate(new Uri("Scene1.xaml", UriKind.RelativeOrAbsolute));
        }
	}
}
