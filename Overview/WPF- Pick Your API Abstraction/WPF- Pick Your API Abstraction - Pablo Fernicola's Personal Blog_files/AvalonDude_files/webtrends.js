...
DCS.dcsaut=dcsGetCookie("XBXGt");
var defHome="default.aspx";
DCSext.wtEvtSrc=DCS.dcssip+DCS.dcsuri;
var srctmp=DCS.dcsuri;
var end=srctmp.length;
var lastslash=srctmp.lastIndexOf("/");
if (lastslash==end-1){DCSext.wtEvtSrc+=defHome;DCS.dcsuri+=defHome;}
else if ((lastslash!=end-1)&&(srctmp.substring(lastslash,end).indexOf(".")==-1)){DCSext.wtEvtSrc+="/"+defHome;DCS.dcsuri+="/"+defHome;}
var srctmp=DCS.dcsuri;
var gDirLevels = 5;
var gFpath = srctmp.substring(srctmp.indexOf('/')+1,srctmp.lastIndexOf('/')+1).toLowerCase();
if(gFpath==''){gFpath="/";}
else{
...
if (meta.name){
if (meta.name.toLowerCase().indexOf('wt.')==0){
...
else if (meta.name.toLowerCase()=='category'){
WT.cg_n=gI18n?dcsEscape(meta.content,I18NRE):meta.content;
}
else if (meta.name.toLowerCase()=='wtsubcategory'){
WT.cg_s=gI18n?dcsEscape(meta.content,I18NRE):meta.content;
if((typeof(WT.cg_n)=="undefined")||(WT.cg_n=="")){
WT.cg_n=WT.cg_s;
}
}
else if (meta.name.toLowerCase()=='trackingurl'){
var tc=meta.content.replace(/^\s+/g,'').replace(/\s+$/g,'');
if(tc.length>0){
var p=tc.split('?');
DCS.dcsuri=gI18n?dcsEscape(p[0],I18NRE):p[0];
if(p.length>1){
DCS.dcsqry=gI18n?dcsEscape(p[1],I18NRE):p[1];
}
}
}
}
