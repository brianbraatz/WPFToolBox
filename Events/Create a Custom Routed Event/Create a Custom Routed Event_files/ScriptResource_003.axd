Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior = function(element) {
    /// <param name="element" domElement="true">The DOM element the behavior is associated with.</param>
    Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior.initializeBase(this, [element]);
...
Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior.prototype = {
    initialize: function() {
        Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior.callBaseMethod(this, 'initialize');
    ...
    get_contentLocale: function() {
        return this._contentLocale;
    },
    set_contentLocale: function(value) {
        this._contentLocale = value;
    },
    
    get_contentVersion: function() {
        return this._contentVersion;
    },
    set_contentVersion: function(value) {
    ...
    dispose: function() {
        ...
        Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior.callBaseMethod(this, 'dispose');
    },
    ...
    initializeCompletionList: function(element) {
        ...
        completionListStyle.position = 'absolute';
        completionListStyle.zIndex = '100';
    ...
    // return false if the popup extends beyond the client area
    _isPopupFullyVisible: function()
    {
      var elem = this._completionListElement;
      ...
      while ( true )
      {
        if ( ! ((top >= parent.scrollTop)
             && (top + height <= parent.scrollTop + parent.clientHeight)) )
        {
          return false;
        }
        ...
        if (this._servicePath && this._serviceMethod) {
            ...
            Sys.Net.WebServiceProxy.invoke(this.get_servicePath(), this.get_serviceMethod(), false,
                                        { prefixText : this._currentPrefix,
                                          ...
                                          contentLocale: this._contentLocale,
    ...
    _setText: function(text) {
        ...
        // If there are multiple tokens, make sure we add the current selection to those already entered
        var currText = element.value;
        var idx = currText.lastIndexOf(' ');
        if ( idx != -1 )
        {
          text = currText.substr(0,idx) + ' ' + text;
        }
    ...
    _update: function(prefixText, completionItems, cacheResults) {
        ...
        if (completionItems && completionItems.length) {
            ...
            // If the popup is going to extend out of the client area, display it above the textbox
            if ( this._isPopupFullyVisible() )
            {
              this._popupBehavior.set_positioningMode( Microsoft.Mtps.Rendering.Behaviors.Ajax.PositioningMode.BottomLeft );
            }
            else
            {
              this._popupBehavior.set_positioningMode( Microsoft.Mtps.Rendering.Behaviors.Ajax.PositioningMode.TopLeft );
            }
...
Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior.descriptor = {
    properties: [   {name: 'completionInterval', type: Number},
                    ...
                    {name: 'contentLocale', type: String},
                    {name: 'contentVersion', type: String} ]
}
Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior.registerClass('Microsoft.Mtps.Rendering.Behaviors.Ajax.TagAutoCompleteBehavior', Microsoft.Mtps.Rendering.Behaviors.Ajax.BehaviorBase);
