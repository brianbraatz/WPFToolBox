...
namespace Hardcodet.Wpf.Samples
{
  ...
  public class CategoryTree : TreeViewBase<ShopCategory>
  {
    ...
    #region DragDrop
    public override void OnItemDrop(TreeViewItem tviSource, ShopCategory objSource, TreeViewItem tviDest, ShopCategory objDest)
    {
        int yy = 99;
        System.Diagnostics.Debug.WriteLine("OnItemDrop");
        if (objSource == null || objDest == null)
            return;

        // remove from source
        objSource.ParentCategory.SubCategories.Remove(objSource);

        objSource.ParentCategory = objDest;

        // more to dest parent
        objDest.SubCategories.Add(objSource);
