  ...
  public abstract partial class TreeViewBase<T>
  {
    ...
    private static void TreePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
      ...
      if (oldValue != null)
      {
        ...
        oldValue.PreviewMouseLeftButtonDown -= owner.OnPreviewMouseLeftButtonDown;
        oldValue.PreviewMouseMove -= owner.OnPreviewMouseMove;
        oldValue.PreviewMouseLeftButtonUp -= owner.OnPreviewMouseLeftButtonUp;
      ...
      if (newValue != null)
      {
        ...
        newValue.PreviewMouseLeftButtonDown += owner.OnPreviewMouseLeftButtonDown;
        newValue.PreviewMouseMove += owner.OnPreviewMouseMove;
        newValue.PreviewMouseLeftButtonUp += owner.OnPreviewMouseLeftButtonUp;
   ...
   //     owner.SelectedItemChanged
    ...
    public abstract void OnItemDrop(TreeViewItem tviSource, T objSource, TreeViewItem tviDest, T objDest);

    private TreeViewItem mtviDragSource;
    private T mObjDragSource;

    private TreeViewItem mtviDragDest;
    private T mObjDragDest;
    bool bMouseDown = false;

    private void InitDragDrop()
    {
        Tree.AllowDrop = true;
        Tree.Drop += new DragEventHandler(TreeViewDrop);
    }
    Point startPoint;

    /*   private TreeViewItem GetTreeViewItemFromPoint(Point point)
       {
           UIElement element = Tree.InputHitTest(point) as UIElement;
           while (element != null)
           {
               if (element == Tree)
               {
    ...
    private void OnPreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        int i = 99;
        T o = this.SelectedItem;
        //       DependencyObject uie = Tree.InputHitTest(e.GetPosition(Tree)) as DependencyObject;
        mtviDragSource = e.Source as TreeViewItem;
        if (mtviDragSource != null)
        {
            mObjDragSource = (T)mtviDragSource.Header as T;
            bMouseDown = true;
        }
        else
        {
    ...
    {
        System.Diagnostics.Debug.WriteLine("OnPreviewMouseMove");
        if (bMouseDown == false)
            return;

        if (Math.Abs(e.GetPosition(Tree).X - startPoint.X) > SystemParameters.MinimumHorizontalDragDistance ||
            Math.Abs(e.GetPosition(Tree).Y - startPoint.Y) > SystemParameters.MinimumVerticalDragDistance)
        {
            if (bMouseDown == true)
            {
                DragDropEffects effects = DragDropEffects.Copy | DragDropEffects.Move;
                DataObject dataObj = new DataObject(mObjDragSource);
                // otherwise kick off a drag drop
                System.Diagnostics.Debug.WriteLine("DragDrop.DoDragDrop");
                try
                {
                    DragDrop.DoDragDrop(Tree, dataObj, effects);
                }
                catch (Exception err) { }
                finally { }
    ...
    private void OnPreviewMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
    {
        ...
        //      DependencyObject uie = Tree.InputHitTest(e.GetPosition(Tree)) as DependencyObject;
        //      mtviDragDest = e.Source as TreeViewItem;
        //       mObjDragDest = (T)mtviDragDest.Header;
    }

    private void TreeViewDrop(object sender, DragEventArgs e)
    {
        System.Diagnostics.Debug.WriteLine("TreeViewDrop");
        bMouseDown = false;
        DependencyObject uie = Tree.InputHitTest(e.GetPosition(Tree)) as DependencyObject;
        mtviDragDest = e.Source as TreeViewItem;
        mObjDragDest = (T)mtviDragDest.Header;
        T objsrc = (T)e.Data.GetData(typeof(T));
        OnItemDrop(mtviDragSource, mObjDragSource, mtviDragDest, mObjDragDest);
        dragcount--;
